...
class ServiceOrderService
{
    ...
    public function placeOrder(Request $request)
    {
        ...
        if (allowOldUnEncryptedOrder() && !$request->has("token")) {
            ...
            if (\Schema::hasColumn("orders", 'fees')) {
                $orderFees = $orderData['fees'] ?? [];
                if(empty($orderFees) && $request->fees){
                    $orderFees = json_encode($request->fees ?? []);
                }

                $orderFees ??= [];
                //convert orderFees to json if it's an array
                if (is_array($orderFees)) {
                    $orderFees = json_encode($orderFees);
                }
