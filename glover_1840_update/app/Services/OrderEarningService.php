    ...
    public function updateEarning($order)
    {
        ...
        if (in_array($order->status, ["delivered", "successful"])) {
            ...
            try {
                ...
                $driverSubscriptionEnabled = (new DriverSubscriptionService())->enabled();
                ...
                if ($order->vendor != null) {
                    ...
                    if (empty($earned)) {
                        ...
                        if (\Schema::hasColumn("orders", 'fees')) {
                            $feesData = $order->fees ?? [];
                            $orderFees = is_string($feesData) ? json_decode($feesData, true) : $feesData;
                            
                            if (is_array($orderFees)) {
                ...
                //update driver
                try {
                    if (!empty($order->driver_id) && !$driverSubscriptionEnabled) {
                        ...
                        if (empty($earned)) {
                            ...
                            if (!$isCashOrder) {
                                //no adding the earning amount when order is cash delivery fee forced
                                if (!$collectDeliveryCash) {
