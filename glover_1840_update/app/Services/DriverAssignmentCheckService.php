...
class DriverAssignmentCheckService
{
    ...
    {
        $this->driverSubscriptionService = new DriverSubscriptionService();
    }
    ...
    public function checkCanAssignOrder($order)
    {
        ...
        if (empty($order)) {
            $errorCode = \Str::random(5);
            Logger("Order check failed", [
                "code" => $errorCode,
                "order_id" => request()->order_id ?? "",
                "request data" => request()->all(),
            ]);
            $errorMessage = __("Order could not be found.");
            $errorMessage .= _("Please share code: $errorCode with support team");
            throw new \Exception($errorMessage);
        }

        $driver = User::find(request()->driver_id);
        if (empty($driver)) {
            throw new \Exception(__("Driver could not be found"), 1);
        }

        $this->driverSubscriptionService->ensureCanReceiveAssignments($driver);
