...
use App\Jobs\DispatchScheduledTaxiOrder;
...
class TaxiOrderController extends Controller
{
    ...
    {
        $this->middleware(['idempotency', 'throttle.order.api'])->only('book');
    }
    ...
    public function book(Request $request)
    {
        ...
        try {
            ...
            if (number_format($vehicleType->total, 2) > number_format($request->total, 2)) {
                //handle coupon discounting
                $couponController = (new \App\Http\Controllers\API\CouponController());
                $couponCodeInfo = $couponController->show($request, $request->coupon_code);
                if ($couponCodeInfo->isSuccessful()) {
                    $couponModel =  $couponCodeInfo->getData(true);
                    if ($couponModel['percentage']) {
                        $calculatedTripTotal = ($couponModel['discount'] / 100) * $vehicleType->total;
                    } else {
                        $calculatedTripTotal = $vehicleType->total - $couponModel['discount'];
                    }

                    if (number_format($calculatedTripTotal, 2) > number_format($request->total, 2)) {
                        throw new \Exception(__("An error occured while trying to book your trip  Amount issue"), 1);
                    }
                } else {
                    $errorMsg = __("An error occured while trying to book your trip  Amount issue");
                    throw new \Exception($couponCodeInfo->getData(true)["message"] ?? $errorMsg, 1);
                }
            ...
            $order->save();
            $order->setStatus($this->getNewOrderStatus($request, true));
            ...
            // For scheduled bookings, dispatch a dedicated delayed job that fires exactly
            // at pickup time instead of relying on the every-5-minute polling job.
            if ($order->status === 'scheduled' && $order->pickup_date && $order->pickup_time) {
                $pickupAt = \Carbon\Carbon::parse($order->pickup_date . ' ' . $order->pickup_time);
                DispatchScheduledTaxiOrder::dispatch($order)->delay($pickupAt);
            }
