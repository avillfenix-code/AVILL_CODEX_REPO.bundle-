...
class OrderDetailsPage extends StatelessWidget {
  const OrderDetailsPage({required this.order, this.isOrderTracking = false, Key? key}) : super(key: key);
  final Order order;
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<OrderDetailsViewModel>.reactive(
      ...
      builder: (context, vm, child) {
        return BasePage(
          title: "Detalle del pedido".tr(),
          showAppBar: true,
          showLeadingAction: true,
          isLoading: vm.isBusy,
          elevation: 0,
          backgroundColor: context.isDarkMode ? context.theme.scaffoldBackgroundColor : Colors.grey.shade50,
          ...
          actions: vm.order.isPackageDelivery
              ? [Icon(FlutterIcons.share_2_fea, color: Utils.textColorByColor(context.theme.colorScheme.surface)).p8().onInkTap(vm.shareOrderDetails).p8()]
              : [],
          body: vm.isBusy
              ...
              : SmartRefresher(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: VStack([
                      ...
                      if (vm.order.deliveryAddress != null) OrderDetailsCard(child: OrderAddressesView(vm)),
                      if (!vm.order.isPackageDelivery && vm.order.deliveryAddress == null)
                        OrderDetailsCard(child: "Recogida del pedido del cliente".tr().text.italic.xl.medium.makeCentered().py(8)),
                      if (_showTrackingCardView(vm)) OrderDetailsCard(child: OrderStatusView(vm).py(4)),
                      OrderDetailsCard(child: OrderDetailsItemsView(vm)),
                      OrderDetailsCard(child: OrderDetailsVendorInfoView(vm)),
                      if (vm.order.driver != null) OrderDetailsCard(child: OrderDetailsDriverInfoView(vm)),
                      if ((vm.order.note.isNotEmpty) || (vm.order.attachments != null && vm.order.attachments!.isNotEmpty))
                        OrderDetailsCard(
                          child: VStack([
                            if (vm.order.note.isNotEmpty) ...[
                              "Nota".tr().text.semiBold.xl.make(),
                              "${vm.order.note}".text.light.sm.maxLines(3).overflow(TextOverflow.ellipsis).make(),
                            ],
                            if (vm.order.attachments!.isNotEmpty) OrderAttachmentView(vm),
                          ], spacing: 10),
                        ),
                      OrderDetailsCard(child: VStack([OrderPaymentInfoView(vm), UiSpacer.divider().py(12), OrderDetailsSummary(vm.order)])),
                      UiSpacer.vSpace(60),
  ...
  bool _showTrackingCardView(OrderDetailsViewModel vm) {
    return vm.order.status != "delivered" && vm.order.status != "failed" && vm.order.status != "cancelled";
  }
}
